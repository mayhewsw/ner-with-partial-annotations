...
class MarginalConditionalRandomField(torch.nn.Module):
    """
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.Tensor, marginals: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        for i in range(1, sequence_length):
            ...
            if marginals is not None:
                alpha = alpha + marginals[:, i, :] * mask[i].view(batch_size, 1)
    ...
    def forward(self,
        ...
        if mask is None:
            ...
            mask = torch.ones((tags.shape[0], tags.shape[1]))

        log_denominator = self._input_likelihood(inputs, mask)
        log_numerator = self._input_likelihood(inputs, mask, marginals=tags)
